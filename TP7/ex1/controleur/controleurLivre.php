...
class ControleurLivre extends ControleurObjet
{
    ...
    public static function modifierLivre()
    {
        ...
        if ($result) {
            ...
            header("Location: routeur.php?controleur=ControleurLivre&action=afficherFormulaireModificationObjet&numLivre=" . $tableau[0]);
        }
    }
    ...
    public static function ajouterAuteurDuLivre()
    {
        ...
        if ($result) {
            header("Location: routeur.php?controleur=controleurLivre&action=definirAuteurs&numLivre=".$tableau[0]);
        } else {
            header("Location: routeur.php?controleur=ControleurLivre&action=afficherFormulaireAjoutAuteurDuLivre&numLivre=" . $tableau[0]);
        }
    }
    ...
    public static function definirAuteurs()
    {
        ...
        foreach ($tabAuteurs as $auteur) {
            $tableauAffichage[] = "<div class='ligne'><div><b>N°". $auteur->get("numAuteur") ."</b> " . $auteur->get("nomAuteur") . " " . $auteur->get("prenomAuteur") . "</div>" . "<a class='bouton btn-red' href='routeur.php?controleur=ControleurLivre&action=supprimerAuteurDuLivre&numLivre=" . $tableau[0] . "&numAuteur=" . $auteur->get("numAuteur") . "'><i class='bi bi-trash'></i></a>" . "</div>";
        }
        $tableauAffichage[] = "<br />";

        foreach ($tabNonAuteurs as $auteur) {
            $tableauAffichage[] = "<div class='ligne'><div><b>N°". $auteur->get("numAuteur") ."</b> " . $auteur->get("nomAuteur") . " " . $auteur->get("prenomAuteur") . "</div>" . "<a class='bouton btn-green' href='routeur.php?controleur=ControleurLivre&action=ajouterAuteurDuLivre&numLivre=" . $tableau[0] . "&numAuteur=" . $auteur->get("numAuteur") . "'><i class='bi bi-plus'></i></a>" . "</div>";
        }
